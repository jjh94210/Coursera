...
######################################################################
# Code for generating random clusters
def gen_random_clusters(num_clusters):
    random_clusters = []
    for dummy_idx in range(num_clusters):
        rand_x = (random.random() - 0.5) * 2.0
        rand_y = (random.random() - 0.5) * 2.0
        random_clusters.append(alg_cluster.Cluster(set([]), rand_x, rand_y, 0, 0.0))
    return random_clusters

size_random_clusters = range(2, 201)
slow_time = []
fast_time = []

for n in range(2, 201):
    random_clusters = gen_random_clusters(n)
    #random_clusters.sort(key = lambda cluster: cluster.horiz_center())
    start1 = time.time()
    slow_closest_pair(random_clusters)
    end1 = time.time() - start1
    slow_time.append(end1)
    start2 = time.time()
    random_clusters.sort(key = lambda cluster: cluster.horiz_center())
    fast_closest_pair(random_clusters)
    end2 = time.time() - start2
    fast_time.append(end2)

plt.figure()
plt.plot(size_random_clusters, slow_time, '-b', label = 'slow_closest_pair')
plt.plot(size_random_clusters, fast_time, '-g', label = 'fast_closest_pair')
plt.legend(loc = 'upper right')
plt.title('Computation time of slow & fast closest pair algorithm(Desktop Python)')
