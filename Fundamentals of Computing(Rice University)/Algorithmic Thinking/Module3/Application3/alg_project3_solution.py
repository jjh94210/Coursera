...
def hierarchical_clustering(cluster_list, num_clusters):
    ...
    while len(copy_cluster_list) > num_clusters:
        copy_cluster_list.sort(key = lambda cluster: cluster.horiz_center())
        closest_pair = fast_closest_pair(copy_cluster_list)
        copy_cluster_list[closest_pair[1]].merge_clusters(copy_cluster_list[closest_pair[2]])
        copy_cluster_list.pop(closest_pair[2])
    return copy_cluster_list
...
def kmeans_clustering(cluster_list, num_clusters, num_iterations):
    ...
    copy_cluster_list2 = list(cluster_list)
    copy_cluster_list2.sort(key = lambda cluster: cluster.total_population())
    copy_cluster_list2.reverse()
    size = len(cluster_list)
    new_clusters = [alg_cluster.Cluster(set([]), copy_cluster_list2[dummy_idx].horiz_center(), copy_cluster_list2[dummy_idx].vert_center(), 0, 0.0) for dummy_idx in range(num_clusters)]
