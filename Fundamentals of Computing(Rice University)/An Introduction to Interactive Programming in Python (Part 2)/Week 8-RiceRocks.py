...
# helper function to update and draw for each sprite in the group
...
# helper function to check for collisions between other object and elements of the group
...
# helper function to check for collisions between elements of the group and elements of the another group
...
def draw(canvas):
    ...
    # process(draw and update) sprites
    process_sprite_group(rock_group, canvas)
    process_sprite_group(missile_group, canvas)
    process_sprite_group(explosion_group, canvas)

    # determine the ship hit any of the rocks
    if group_collide(rock_group, my_ship):
        lives -= 1

    # detect missile/rock collisions and update score
    score = score + 10 * group_group_collide(rock_group, missile_group)

    # game reset when there is no lives.
    if lives == 0:
        rock_group = set([])
        started = False
        soundtrack.rewind()
...
def rock_spawner():
    ...
    rock_vel = [random.randrange(0, score + 10) / 100.0 * .6 - .3, random.randrange(0, score + 10) / 100.0 * .6 - .3]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if dist(my_ship.get_position(), rock_pos) <= 1.5 * (my_ship.get_radius() + a_rock.get_radius()):
        return
...
# http://www.codeskulptor.org/#user43_ARB4Vuis2Q_0.py
