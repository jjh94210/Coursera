...
def average_in_degree(normalized_distribution):
    average = 0.0
    for dummy_degree in normalized_distribution.keys():
        average += dummy_degree * normalized_distribution[dummy_degree]
    return average

def average_out_degree(digraph):
    sum_of_out_degree = 0
    for dummy_node in digraph.keys():
        sum_of_out_degree += len(digraph[dummy_node])
    return (float(sum_of_out_degree) / len(digraph.keys()))
...
print average_in_degree(normalized)
